    ...
    void Update()
    {
        ...
        if (!isAttacking)
        {
            if (distanceToPlayer <= detectRange)
            {
                ...
                rb.linearVelocity = Vector2.zero; // D?NG di chuy?n hoàn toàn
                movement = Vector2.zero;

                // H??ng v? phía player
                Vector2 dir = (player.position - transform.position).normalized;

                // L?t h??ng sprite (n?u dùng flipX thay v́ animation trái/ph?i)
                if (dir.x < 0)
                    spriteRenderer.flipX = true;
                else if (dir.x > 0)
                    spriteRenderer.flipX = false;

                // N?u t?i lúc t?n công th́ t?n công
                if (attackTimer <= 0f)
                {
            ...
            else
            {
                ...
                // Cho nhân v?t di chuy?n b́nh th??ng khi không t?n công
                rb.linearVelocity = movement * speed;
            }
        }
        else
        {
            // ?ang t?n công th́ KHÔNG di chuy?n
